#!/bin/bash
# Radiance vLLM rootful launcher — Qwen3.8-27B INT4 on GPU1 (single-card test)
# Rootful (sudo podman) because rootless memlock hard limit is 8MB and HSA needs unlimited.
set -e
mkdir -p /srv/ai/wip/radiance-cache
chmod 777 /srv/ai/wip/radiance-cache
podman rm -f vllm-radiance >/dev/null 2>&1 || true
